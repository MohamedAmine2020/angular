...
export class CompleteProfileService implements Resolve<any> {
    ...
    public completeProfileAfterAuthPhone(worker: WorkerProfileModel, personalInformation: PersonalInformation): Promise<void> {
        return new Promise((resolve, reject) => {
            Promise.all([
                this._workerService.create(worker).toPromise(),
                this._accountService.updateAccount(personalInformation)
            ]).then(
                () => {
                    resolve();
                },
